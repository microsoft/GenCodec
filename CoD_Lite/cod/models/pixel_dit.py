...
class FlattenDiTBlock(nn.Module):
    ...
    def forward(self, x, c, pos, mask=None, use_checkpoint=False):
        if use_checkpoint:
            return checkpoint(self._forward, (x, c, pos, mask))
        else:
...
class NerfBlock(nn.Module):
    ...
    def forward(self, x, s, use_checkpoint=False):
        if use_checkpoint:
            return checkpoint(self._forward, (x, s))
        else:
            return self._forward((x, s))


class PixNerDiT(CoDBase):
    def __init__(
            self,
            pred='v',
            in_channels=4,
    ...
    def forward(self, x, t, y, cond, s=None, mask=None, return_codec_res=False, return_pred=False):
        input_x = x
        input_t = t
        ...
        if s is None:
            ...
            for i in range(self.num_cond_blocks):
                s = self.blocks[i](s, c2, pos, mask, self.use_checkpoint)
