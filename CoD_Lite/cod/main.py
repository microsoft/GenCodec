...
def micro_batch_collate_fn(batch):
...
class DataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        ...
        if micro_batch_size is not None:
            ...
            train_collate_fn = micro_batch_collate_fn
        else:
    ...
    def val_dataloader(self) -> EVAL_DATALOADERS:
        sampler = torch.utils.data.distributed.DistributedSampler(
            self.eval_dataset, num_replicas=self.trainer.world_size,
            rank=self.trainer.global_rank, shuffle=False)
        return DataLoader(self.eval_dataset, self.eval_batch_size,
                          num_workers=self.eval_num_workers, prefetch_factor=2,
                          sampler=sampler, collate_fn=eval_collate_fn)

    def predict_dataloader(self) -> EVAL_DATALOADERS:
        sampler = torch.utils.data.distributed.DistributedSampler(
            self.pred_dataset, num_replicas=self.trainer.world_size,
            rank=self.trainer.global_rank, shuffle=False)
        return DataLoader(self.pred_dataset, batch_size=self.pred_batch_size,
                          num_workers=self.pred_num_workers, prefetch_factor=4,
                          sampler=sampler, collate_fn=eval_collate_fn)
...
class LightningModel(pl.LightningModule):
    ...
    def state_dict(self, *args, destination=None, prefix="", keep_vars=False):
        ...
        self.denoiser.state_dict(destination=destination, prefix=prefix+"denoiser.", keep_vars=keep_vars)
        self.ema_denoiser.state_dict(destination=destination, prefix=prefix+"ema_denoiser.", keep_vars=keep_vars)
        self.diffusion_trainer.state_dict(destination=destination, prefix=prefix+"diffusion_trainer.", keep_vars=keep_vars)
        return destination
    ...
    def instantiate_trainer(self, **kwargs: Any) -> Trainer:
        ...
        if not self.resume_path and os.path.exists(default_root_dir):
            ...
            if len(ckpts) > 0:
                ...
                # Merge split DDP checkpoint parts
                ...
                print(f"[Resume Training] checkpoint: {latest_ckpt}")
